...
def enhance_contrast_batch(images, edge_maps, intensity=2.0, edge_threshold=0.1):
    """
    Enhance contrast in regions guided by edge_map with smooth blending for batches.

    Args:
        images (np.ndarray): Batch of input images (B, H, W, C) in BGR or grayscale
        edge_maps (np.ndarray): Batch of edge maps (B, H, W)
        intensity (float): Contrast enhancement intensity
        edge_threshold (float): Weak edge filter threshold

    Returns:
        np.ndarray: Contrast-enhanced images with same shape as input
    """
    # Convert to float32 for processing
    images = np.asarray(images, dtype=np.uint8)
    edge_maps = np.asarray(edge_maps, dtype=np.float32)
    
    batch_size = images.shape[0]
    enhanced_batch = []
    
    # Create CLAHE once for efficiency
    ...
    for i in range(batch_size):
        ...
        edge_norm[edge_norm < edge_threshold] = 0
        edge_norm = cv2.GaussianBlur(edge_norm, (5, 5), 0)
        ...
        # Smooth blending
        blend_weight = edge_norm[..., np.newaxis] if img.ndim == 3 else edge_norm
        result = img * (1 - blend_weight) + enhanced * blend_weight
        enhanced_batch.append(result.astype(np.uint8))
    
    return np.stack(enhanced_batch, axis=0)


def enhance_contrast(image, edge_map, intensity=1.5):
    """
    Enhance contrast in regions guided by edge_map.
    
    Args:
        image (list): Input image as a list (will be converted to a NumPy array).
        edge_map (list): Edge map as a list (will be converted to a NumPy array).
        intensity (float): Scaling factor for contrast enhancement.
    ...
    # cv2.imwrite(f'/data/mzh/SonarNewSegmentation/segmentation-guided-diffusion/edgs/test_yuanshi.png', image)
    # Ensure edge_map is normalized to [0, 1]
    edge_map = edge_map / edge_map.max() if edge_map.max() > 0 else edge_map

    # Create enhancement mask
    enhancement_mask = np.clip(edge_map, 0, 1)

    # Apply contrast enhancement
    # enhanced_image = cv2.convertScaleAbs(image, alpha=intensity, beta=0)  # Increase contrast
    # enhanced_image[enhancement_mask > 0] = enhanced_image[enhancement_mask > 0]

    enhanced_image = cv2.convertScaleAbs(image, alpha=intensity, beta=0)
    result = image.copy()
    result[enhancement_mask > 0] = enhanced_image[enhancement_mask > 0]
    return result

    return enhanced_image
    
def compute_fused_from_images(images):
    features = []
    for img in images:
        img_mean = np.mean(img, axis=(0, 1)) / 255.0  
        features.append(img_mean)
    
    features_np = np.array(features)  # (N, 3)
    
    fused_features = np.mean(features_np, axis=0)  # (3,)
    
    fused_rgb = np.tile(fused_features, (256, 256, 1))  # (256, 256, 3)
    
    fused_tensor = torch.tensor(fused_rgb.transpose(2, 0, 1), dtype=torch.float32).unsqueeze(0).to(device)
...
def compute_edge_density(batch_images):
    ...
    for image in batch_images:
        ...
        # print(f"Image dtype: {image.dtype}, shape: {image.shape}")
        # print(f"Image min: {image.min()}, max: {image.max()}")
        cv2.imwrite(f'/data/mzh/SonarNewSegmentation/segmentation-guided-diffusion/edgs/test_yuanshi_{count}.png', image)
        edges = cv2.Canny(image, 80, 200)
        # print(f"edges dtype: {edges.dtype}, shape: {edges.shape}")
        # print(f"edges min: {edges.min()}, max: {edges.max()}")
        cv2.imwrite(f'/data/mzh/SonarNewSegmentation/segmentation-guided-diffusion/edgs/test_edges_{count}.png', edges)
...
def evaluate_sample_many(
    ...
    sample_dir = test_dir = os.path.join(config.output_dir, "test_many_{}".format(sample_size))
    ...
    for bidx, seg_batch in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
        if num_sampled < sample_size:
            ...
            seg = seg_batch['seg_all']
            seg[seg>0]=1
            # print(f"seg dtype: {seg.dtype}, shape: {seg.shape}")
            # print(type(images))
            # print(f"images dtype: {images.dtype}, shape: {images.shape}")
            edge_densities,edge_map = compute_edge_density(seg)
            # print(type(edge_map))  
            # print(f"edge_map dtype: {edge_map.dtype}, shape: {edge_map.shape}")
            enhanced_image = enhance_contrast_batch(images, edge_map, intensity=2.0)
            # print(type(enhanced_image))
            # print(f"enhanced_image dtype: {enhanced_image.dtype}, shape: {enhanced_image.shape}")
            # enhanced_image = enhanced_image.tolist()
            ...
            for i in range(enhanced_image.shape[0]):
                ...
                cv2.imwrite(img_fname, img_to_save)
...
def evaluate(config, epoch, pipeline, seg_batch=None, class_label_cfg=None, translate=False):
    ...
    sample_dir = test_dir = os.path.join(config.output_dir, "samples_many_{}".format(epoch))
    if not os.path.exists(sample_dir):
        os.makedirs(sample_dir)
    num_sampled = 0
    ...
    for i, img in enumerate(images):
        if config.segmentation_guided:
            ...
            img_fname = f"{test_dir}/{os.path.splitext(seg_batch['image_filenames'][i])[0]}_pre.png"
            # save_image(img, f"{test_dir}/{os.path.splitext(seg_batch['image_filenames'][i])[0]}_pre.png", normalize=True)
            # for seg_type in seg_batch.keys():
            #     if seg_type.startswith("seg_"):
            save_image(seg_batch["seg_all"][i], f"{test_dir}/{os.path.splitext(seg_batch['image_filenames'][i])[0]}_seg.png", normalize=True)
            # img_og = seg_batch['images']
            save_image(seg_batch["images"][i], f"{test_dir}/{os.path.splitext(seg_batch['image_filenames'][i])[0]}_ori.png", normalize=True)
        else:
            img_fname = f"{sample_dir}/{num_sampled + i:04d}.png"
        img.save(img_fname)

    num_sampled += len(images)
    print("sampled {}/{}.".format(num_sampled, config.eval_batch_size))
...
def feature_fusion_method1(features, image):
    # features: [9,1,256,256]
    # image: [9,4,256,256]
    
    expanded_features = features.expand(-1, 4, -1, -1)  # [9,4,256,256]
    
    alpha = 0.7  
    fused = alpha * image + (1 - alpha) * expanded_features
    
    return fused  # [9,4,256,256]
class SegGuidedDDIMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            # first, concat segmentations to noise
            image = add_segmentations_to_noise(image, seg_batch, self.external_config, self.device)
            # fusion_model = ChannelAttentionFusion()
            # fusion_model = fusion_model.to(device)
            # res_block = SameShapeResBlock(in_channels=1)
            # res_block.eval()
            # features = res_block(seg_batch["seg_all"])
            # features = features.to(device)
            # image = image.to(device) 
            # # fused = torch.mean(features, dim=0, keepdim=True)
            # image = feature_fusion_method1(features, image)
            # image = fusion_model(features,image)

            alpha = 1.0 - (t / num_inference_steps) 
            multiclass_masks_shape = (image.shape[0], 1, image.shape[2], image.shape[3])
            segs = convert_segbatch_to_multiclass(multiclass_masks_shape, seg_batch, self.external_config, self.device) 
            # concat segs to noise
            noisy_images = torch.cat((noisy_images, segs), dim=1)
            noisy_images = (1 - alpha) * noisy_images + alpha * segs

            mixed_seg = (1 - alpha) * image[:, :1, :, :] + alpha * segs 
            # mixed_seg = (1 - alpha) * noisy_images + alpha * segs.repeat(1, C, 1, 1)  

            image = torch.cat((image, mixed_seg), dim=1)  
