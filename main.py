...
def denoise_image(pil_img: Image.Image) -> Image.Image:
    ...
    denoised_img_cv = cv2.fastNlMeansDenoising(img_cv, None, 10, 7, 21)
    return Image.fromarray(denoised_img_cv)
...
def augment_fn(example):
    ...
    pil_img = denoise_image(pil_img)

    pil_tensor = transform(pil_img)

    example["image"] = pil_tensor.numpy().transpose(1, 2, 0)  
    example["image"] = (pil_tensor * 255).numpy().astype(np.uint8).squeeze()
    return example
...
def main(
    ...
    if config.segmentation_guided:
        ...
        # dataset_train = dataset_train.map(augment_fn)
        dataset_eval = datasets.Dataset.from_dict(dset_dict_eval)
        # dataset_eval = dataset_eval.map(augment_fn)
    ...
    if (mode == "train" and resume_epoch is not None) or "eval" in mode:
        ...
        print(os.path.join(config.output_dir, 'unet'))
